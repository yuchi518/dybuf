...
- [x] Java 同步常數並完成 typdex wire-layout 相容性重構
...
- [x] 執行 C、Python、JavaScript、Java 測試及跨語言 fixture 驗證

### Findings

- Canonical public name: `TYPDEX_TYP_OBJ = 0x0e`.
- Python, JavaScript, and Java export the canonical `TYPDEX_TYP_OBJ` name without
  an object alias.
- C/dypkt exposes the corresponding `dype_obj`; Java verifies typdex encoding
  against C fixtures.
...
- [x] 建立 Python/JavaScript/Java 共用 JSON fixture
- [x] 透過共用 fixture 驗證 Python、JavaScript、Java 可 decode C 產生的 JSON-dybuf bytes
- [x] 透過共用 fixture 驗證 Python、JavaScript、Java re-encode 到相同 JSON-dybuf bytes
...
- Java varint bucket encoding大致與 C 一致，現已由完整 golden fixtures 覆蓋。
- Java typdex 原本使用舊 layout；例如 C canonical `83ff` 應 decode 為
  `type=3,index=255`，Java 目前 decode 為 `type=1,index=511`。
- Java `putTypdex(type=63,index=255)` 原本輸出 `df80ff`，C canonical 是 `bfff`。
- Java var-length empty bytes/string 原本 decode 為 `null`，與 Python/JavaScript
...
## Java JSON-dybuf helper

- [x] 實作 Java `DyBuf.encodeJson` / `DyBuf.decodeJson`
- [x] 實作 Java `appendJsonValue` / `nextJsonValue` stream helpers
- [x] Java JSON value model 使用 `Map<String,Object>`、`List<Object>`、`String`、
  `Boolean`、`Long`、`Double`、`null`
- [x] Java object/map encode 使用 `LinkedHashMap` 保留 key order
- [x] Java decode 端拒絕非 finite double
- [x] Java golden test 讀取 `fixtures/v1/json_values.json`
- [x] Java golden test 驗證 JSON-dybuf decode value 與 encode bytes
- [x] Java deterministic random JSON round-trip 測試
